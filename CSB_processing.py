...
def isolate_gebco_direct_measurements(bathy_path, tid_path, output_path):
    print("Filtering GEBCO using TID grid...")
    with rasterio.open(bathy_path) as src_bathy, rasterio.open(tid_path) as src_tid:
        bathy_data = src_bathy.read(1)
        meta = src_bathy.meta.copy()
        nodata_val = src_bathy.nodata if src_bathy.nodata is not None else -32767.0
        
        tid_resampled = np.empty(bathy_data.shape, dtype=np.uint8)
        
        reproject(
            source=rasterio.band(src_tid, 1),
            destination=tid_resampled,
            src_transform=src_tid.transform,
            src_crs=src_tid.crs,
            dst_transform=src_bathy.transform,
            dst_crs=src_bathy.crs,
            resampling=rioResampling.nearest 
        )
        
        direct_measurement_mask = (tid_resampled >= 10) & (tid_resampled <= 17)
        valid_bathy_mask = (bathy_data != nodata_val)
        keep_mask = direct_measurement_mask & valid_bathy_mask
        
        filtered_depths = np.full(bathy_data.shape, nodata_val, dtype=np.float32)
        filtered_depths[keep_mask] = bathy_data[keep_mask]
        
        meta.update({"driver": "GTiff", "dtype": "float32", "count": 1, "nodata": nodata_val, "compress": "lzw"})
        
        with rasterio.open(output_path, "w", **meta) as dest:
            dest.write(filtered_depths, 1)
...
def loadCSB():
    ...
    df = df[(df['depth'] > 600) & (df['depth'] < 11000)]
...
def tides():
    ...
    # --- NEW: Check if the user wants to skip tide data entirely ---
    # If FES is disabled and no Tide Zone shapefile is specified, bypass tide corrections.
    if not fes_model_var.get() and (not fp_zones_var.get() or fp_zones_var.get().strip() == ""):
        print("***** Skipping Tide Correction: Using raw depths as depth_new *****")
        csb_corr = gdf.copy()
        csb_corr['depth_new'] = csb_corr['depth']
        csb_corr['depth_old'] = csb_corr['depth']
        csb_corr['ControlStn'] = 'None'
        csb_corr['time'] = csb_corr['time'].dt.strftime("%Y%m%d %H:%M:%S")
        csb_corr = gpd.GeoDataFrame(csb_corr, geometry='geometry', crs='EPSG:4326')
        return csb_corr
    ...
    if fes_model_var.get():
        ...
        # Automatically align mismatched CRS
        if zones.crs != gdf.crs:
            print(f"Aligning tide zones CRS ({zones.crs}) to match CSB data CRS ({gdf.crs})...")
        ...
        if tdf:
            ...
            print("No tide data available for the specified period.")
            csb_corr = gpd.GeoDataFrame(columns=gdf.columns.tolist() + ['depth_old', 'depth_new'], crs=gdf.crs)
...
def BAGextract():
    ...
    # --- NEW LOGIC: Filter by TID if the GUI checkbox is checked ---
    if filter_direct_var.get() and tid_filepath_var.get():
        print("User selected Direct Measurements Only. Applying TID filter...")
        filtered_temp_path = os.path.join(output_dir, title + '_direct_only_temp.tif')
        isolate_gebco_direct_measurements(BAG_filepath, tid_filepath_var.get(), filtered_temp_path)
        input_for_warp = filtered_temp_path

    print("Warping input raster to standard WGS84 (EPSG:4269)...")
    
    if input_for_warp.lower().endswith('.bag'):
        print("BAG file detected, creating temporary VRT to select bands 1 and 2...")
        gdal.BuildVRT(temp_vrt_path, input_for_warp, bandList=[1, 2])
        input_for_warp = temp_vrt_path
    ...
    # Clean up the temporary filtered file if we made one
    if filter_direct_var.get() and os.path.exists(input_for_warp) and input_for_warp != BAG_filepath:
        os.remove(input_for_warp)
...
def run_apply_best_offsets(db_path):
    ...
    # --- NEW: GEBCO Reference Outlier Detection ---
        if filter_direct_var.get():
            print("Strict GEBCO Mode: Flagging outliers based on reference bathymetry deviation...")
            
            # NOTE: Raster_Value is multiplied by -1 to match the sign convention of depth_mod
            gebco_outlier_query = """
            UPDATE csb
            SET Outlier = TRUE
            WHERE Raster_Value IS NOT NULL 
              AND ABS((Raster_Value * -1) - depth_mod) > GREATEST(2.0, depth_mod * 0.05);
            """
            con.execute(gebco_outlier_query)
            
            outlier_count_df = con.execute("SELECT COUNT(*) FROM csb WHERE Outlier = TRUE").fetchdf()
            outlier_count = outlier_count_df.iloc[0, 0]
            print(f"Flagged {outlier_count} total spatial outliers against GEBCO reference.")
...
def run_export_transits(db_path, exports_folder):
    ...
    def detect_jumps(group, window=5, abs_threshold=None, rel_threshold=0.10, min_depth=2.0):
        ...
        A point is flagged as a jump outlier only when it deviates significantly
        from BOTH the backward median AND the forward median.  This preserves
        legitimate steep seafloor features (which are consistent with one side)
        ...
        jump_indices = group.index[jump_flags]
...
def process_csb_for_directory(csb_directory, root_window):
    ...
    # Safely schedule the window destruction on the main thread
...
filter_direct_var = tk.BooleanVar(value=False)
tid_filepath_var = tk.StringVar()
...
# --- NEW GUI ELEMENTS ---
filter_direct_checkbox = ttk.Checkbutton(ref_bathy_frame, text="Filter to Direct Measurements Only (Requires GEBCO TID File)", variable=filter_direct_var)
filter_direct_checkbox.grid(row=2, column=0, columnspan=3, sticky='w', padx=5, pady=2)

ttk.Label(ref_bathy_frame, text='GEBCO TID File').grid(row=3, column=0, sticky='w', padx=5, pady=2)
tid_filepath_entry = ttk.Entry(ref_bathy_frame, textvariable=tid_filepath_var, width=60)
tid_filepath_entry.grid(row=3, column=1)
ttk.Button(ref_bathy_frame, text='Browse', command=lambda: open_file_dialog(tid_filepath_var, [("TID or GeoTIFF file", ("*.tid", "*.tif", "*.tiff"))])).grid(row=3, column=2, padx=5)
